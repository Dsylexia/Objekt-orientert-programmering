...
int main () {
	...
	while (kommando != 'Q') {		//Runner så lenge ikke 'Q'
		switch (kommando) {			//Alle muligheter vises under
			case 'N': nyRute();		break;
...
void skrivStopp() {
	cout << "\tAlle busstopp\n";
	for (int i = 0; i < gBusstopp.size(); i++) { // Looper gBusstopp.size()
...
void ruteSkrivData(const Rute rute) {
	...
	cout << "\tBusstopp:  ";
	for (int i = 0; i < rute.stopp.size(); i++) {	//Looper gjennom
		cout << rute.stopp[i] <<" ";				//Skriver ut
		if (i < rute.stopp.size()-1) {	//sørger for at det ikke blir for
			cout << " --> ";			//mange piler '-->'
		}
	}
...
void nyRute(){
	...
	if (ruteLesData(*nyRute)){		//Hvis returnerer ture
	...
	//Hvis ruteLesData ikke rturnerer true
		...
		delete nyRute;
		cout<< "\tAntall ruter lagty til: " 
			<< gRuter.size();	//Sjekker at det ikke blir lagt til noe
...
bool ruteLesData(Rute & rute) {
	...
	while(y != 0) {								//Looper så lenge ikke 0
...
void slettRuter() {
	...
	if (gRuter.size() == 0) {	//Hvis det ikke finnes noen ruter
		cout << "Ingen ruter :( \n";
	}
	else {						//Ellers skjer dette
		skrivRuter();
		...
		if(svar == -1) {slett();} //Hvis '-1' slettes alt
		else if (svar == 0) {	  //Hvis '0' Slettes ingen ting
			cout <<"\tIngen ting slettes\n";
		}
		else {				 	 //Ellers sendes spesifik rute til 
			slett(svar);		 //slett hvor den slettes
		}
	}
};
...
void slett() {
	...
	while(!gRuter.empty()) {			//Looper så lenge den ikke er tom
 ...
 */
void slett(const int nr) { //Looper nr antall ganger
	cout <<"\tSletter rute nummer: " << nr << "\n";	
	delete gRuter[nr];					//Frigjør memory
	gRuter[gRuter.size()] = gRuter[nr]; //Flytter bakerste rute
